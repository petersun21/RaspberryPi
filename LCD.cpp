...
// ---------------------------------------------------------------------------
// Constructor
LCD::LCD (uint8_t I2C_address)
{
	_fd =  wiringPiI2CSetup (I2C_address);
...
void LCD::begin(uint8_t cols, uint8_t lines, uint8_t dotsize)
{
   ...
   // -------------------------------------
    // this is according to the hitachi HD44780 datasheet
      ...
      command(_displayfunction);
      delayMicroseconds(10);  // wait more than 4.1ms
      char busy = (char) wiringPiI2CReadReg8 (_fd, DATA);
      busy = busy & 0x80;
      if(busy > 0) {
    	  std::cout << "line is busy";
      }
...
{
   value &= 0xF0;
   wiringPiI2CWrite(_fd, (value & 0xf0) | 0x08  );

   delay(5);
   wiringPiI2CWrite(_fd, ((value & 0x0f) << 4)| 0x08  );
}
...
void LCD::send(uint8_t value, uint8_t mode) {
	uint8_t half = value & 0xf0;
	wiringPiI2CWrite(_fd, half | 0x09);
	delay(2);
	half = ((value & 0x0f) << 4) & 0xf0;
	wiringPiI2CWrite(_fd, half | 0x09);
}
